            ...
            <div id="operateContainerallBlock">
                ...
                <div id="operateContainermiddleBlock" >
                    ...
                    权限<select id="userLevelselect" name="userLevelselect" style="margin-left:0px"><!-- 向右调整一下 -->
                        ...
                        <input id="userAddandEditbutton" name="userAddandEditbutton" type="submit" style="margin-left: 10px;margin-right: 20px;" value="添加/修改用户" />
                    </form>
                </div>
                <div id="operateContainerrightBlock" >
                    <form action="" method="post" style="position: relative;float: right;margin-left: 10px;">
                        <div>
                            <input id="fileRearchInput" name="fileResearchInput" type="text" style="margin:10px;" value="<?php echo $fileResearchInput; ?>" />
                            <input id="fileRearchButton" name="fileResearchButton"  type="submit" style="margin-left: 10px;margin-right: 20px;" value="搜索" />
                        </div>
                    </form>
                </div>
            </div>
        ...
        <div class="mainContainer"><!-- 页面调转菜单栏右侧的主要操作区样式 -->
            <form id="userListForm" action="" method="post">
                <?php
                    ...
                    $rs=mysqli_query($link,"select * from $userTablename $extraCondition");$i=0;
                    while($row=mysqli_fetch_assoc($rs)){
                    echo "<div class='userListitem' ><!-- 列表中一个用户元素 -->";
                    echo "  <div id='userListitemBlock'>";
                    echo "      <div style='position: relative; float: left;width: 28px;height: 28px;border: #00ff00 solid 1px;'>";
                    ...
                    echo "      </div>";
                    echo "      <div style='position: relative; float: left;width: 28px;height: 28px;border: #00ff00 solid 1px;'>";
                    echo "          <img src='image/icon/user.png' height='25px' width='25px' style='margin: auto;position: absolute;top: 0;left: 0;bottom: 0;right: 0;'>";
                    echo "      </div>";
                    echo "      <span style='margin-left: 6px;margin-top: 6px;position: relative;float: left;font-size: 12px;color:#000000'>用户名：$row[username] $row[level]级管理权限</span>";
                    ...
                    echo "          <input type='button' id='userPasswordreset-0' value='重置密码' />";
                    echo "      </div>";
                    echo "  </div>";
                    echo "</div>";
                    $i++;//input的Id编码
